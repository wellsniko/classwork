...
def harrison_ford
  ...
  Movie.joins(:actors, :castings).where(actors: {name: 'Harrison Ford'}).where.not(castings:{ord: 1}).select(:id, :title).distinct
end
...
def biggest_cast
  ...
  Movie.joins(:castings).group(:id).order("count(*) DESC").select(:title, :id).limit(3)
end

#  Chirp.joins(:likes).group(:id).having("count(*)>2")
...
def directed_by_one_of(them)
  ...
  #movies joins with director, looing at the names of actors and checking if any of
  #"them" are in the name column, return the movie id/title if true
